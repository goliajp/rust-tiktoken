// Brand marks for the registries this project publishes to.
//
// lucide dropped its brand icons, so these come from simple-icons (CC0-1.0),
// extracted once rather than pulled in as a dependency — three paths do not
// justify a package. crates.io has no simple-icons entry; it uses lucide's
// Package, which is what a crate is.
//
// Regenerate: npx simple-icons@latest, read .path off siGithub / siNpm /
// siDocsdotrs.
...
export function Brand({ name, size = 14 }: { name: keyof typeof PATHS | string; size?: number }) {
  const d = PATHS[name]
  if (!d) return null
  return (
    <svg
